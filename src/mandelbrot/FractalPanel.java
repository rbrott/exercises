...
public class FractalPanel extends JPanel implements MouseListener, MouseMotionListener {
	...
	private int computePoint(double real, double imag) {
		double nextReal = 0;
		double nextImag = 0;
		for(int k = 0; k < MAX_ITERATIONS; k++) {
			double temp = nextReal;
			nextReal = temp * temp - nextImag * nextImag + real;
			nextImag = 2 * temp * nextImag + imag;
			if (Math.abs(nextImag) > 2.0 || Math.abs(nextReal) > 2.0) {
				return k;
			}
		}
		return -1;
	...
	public void computeSet(int[][] set) {
		...
		for (int i = 0; i < width; i++) {
			for (int j = 0; j < height; j++) {
				set[i][j] = computePoint(x + (i + 0.5) * hSpacing, y + (j + 0.5) * vSpacing);
			}
		}
	}
